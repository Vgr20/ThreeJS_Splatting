...
// function ViewerIndependenceSquare() {
...
//     // Step 2: Scene load
//     viewer
//       .addSplatScene("/splats/independence_square.splat", {
...
// export default ViewerIndependenceSquare;
...
import { useEffect, useRef } from "react";
import { PointerLockControls } from "three/examples/jsm/controls/PointerLockControls";
import { createBoxColliders } from "./independence_square_colliders";
...
function ViewerIndependenceSquare() {
  ...
  useEffect(() => {
    ...
    camera.position.copy(
      // new THREE.Vector3().fromArray([-0.15829, -0.08684, 0.948])
      new THREE.Vector3().fromArray([0, 0.15, 5.5])
    );
    camera.up = new THREE.Vector3().fromArray([0, 1, 0]).normalize();
    camera.lookAt(new THREE.Vector3().fromArray([-1.36147, 0.01385, -1.93057]));
    ...
    const capsuleHeight = 5;
    const playerBody = new CANNON.Body({
      mass: 1,
      fixedRotation: true,
      // position: new CANNON.Vec3(-0.15829, -0.08684, 0.948),
      position: new CANNON.Vec3(0, 0.15, 5.5),
